...
.setDescription(`<a:flushed:750076071721828452>  **| Müzik oynatabilmek için bir ses kanalında olmanız gerekmektedir!**`)
...
.setDescription(`<a:flushed:750076071721828452>  **| Müzik oynatabilmek için aranacak kelime veyahut kelimeler giriniz!**`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());    
...
const sarki = await client.player.addToQueue(message.guild.id, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("TRK | Kuyruğa Ekle")
  .setDescription(`<a:arrow_forward:750076071721828452> **|** \`${sarki.name}\` **adlı müzik,**\n${message.author} **tarafından kuyruğa eklendi!**`)
.setThumbnail(`https://drummofficial.com/wp-content/uploads/2017/08/equalizer3.gif`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());   
...
const sarki = await client.player.play(message.member.voice.channel, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("TRK | Oynatılan Şarkı")
  .setDescription(`<a:musical_note:750076071721828452> **|** \`${sarki.name}\` **adlı müzik şu anda oynatılıyor.**`)
...
sarki.queue.on('end', () => {
  ...
  .setColor(AloneDogru)
  .setTitle("TRK | Kuyruk Bitti!")
  .setDescription(`<a:stuck_out_tongue:750076071721828452> **| Kuyruktaki tüm müzikler oynatıldı. TRK kanaldan ayrılıyor.\n\TRK Tercih ettiğiniz için teşekkür ederiz.**`)
  .setThumbnail(client.user.avatarURL())
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());  
...
module.exports.config = {
    name: "oynat",
    aliases: ["çal","play","çalkeke","bedoyusik"]
};