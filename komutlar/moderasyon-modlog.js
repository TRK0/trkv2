...
let logkanal = await db.fetch(`log_${message.guild.id}`)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
    if(!logkanal) return message.channel.send(`<a:screwdriver:750076071721828452> **Modlog Kanalı Zaten ayarlı değil**`);
    db.delete(`log_${message.guild.id}`)
   message.channel.send(`<a:screwdriver:750076071721828452> **ModLog Kanalı başarıyla sıfırlandı.**`);
  
    return
  }
  
if (!logk) return message.channel.send(`<a:tools:750076071721828452> **Bir modlog kanalı belirtmelisin.**`);
