...
const emmmmbed = new Discord.MessageEmbed()
  ...
  .addField(
    `TRK | Teşekkürler`,
    `**Selamlar, Ben Lord Creative (TRK'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`
  )
  .addField(
    `Alone | Prefix`,
    `**TRK Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`
  )
  .addField(
    `TRK | Nasıl Kullanılır?`,
    `**TRK botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`
  )
  .addField(`TRK | Linkler`, `**Sohbet Kanalına -davet Yazmanız Yeterlidir**`)
  .setFooter(`TRK | Gelişmiş Türkçe Bot | 2020`)
  .setTimestamp();
...
client.login(fynx.fynxtoken).then(
  function() {
...
client.on("message", async msg => {
  ...
  if (msg.content == `<@!713713727794446397>`)
    return msg.channel.send(
      `> **TRK | Prefix**\n\n> <a:stuck_out_tongue:750076071721828452> **Sanırım beni etiketlediniz.**\n > <a:stuck_out_tongue:750076071721828452> Buyurun prefix(ön ek)im \`${prefix}\``
    );
});
...
client.on("messageDelete", async message => {
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setDescription(
      `<a:stuck_out_tongue:750076071721828452> <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`
    )
    .setFooter("TRK Bot | Log Sistemi");
...
client.on("guildBanAdd", async message => {
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setDescription(
      `<a:stuck_out_tongue:750076071721828452> **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`
    )
...
client.on("channelCreate", async channel => {
  ...
  if (channel.type === "text") {
    let embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        `<a:stuck_out_tongue:750076071721828452> ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`
      )
      .setFooter(`TRK Bot | Log Sistemi Kanal ID: ${channel.id}`);
    modlogkanal.send({ embed });
  }
  if (channel.type === "voice") {
    let embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        `<a:stuck_out_tongue:750076071721828452> ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`
      )
      .setFooter(`TRK Bot | Log Sistemi Kanal ID: ${channel.id}`);
...
client.on("channelDelete", async channel => {
  ...
  if (channel.type === "text") {
    let embed = new Discord.MessageEmbed()
      .setColor("#ffd100")
      .setDescription(
        `<a:stuck_out_tongue:750076071721828452> ${channel.name} **Adlın Metin Kanalı  Silindi**`
      )
      .setFooter(`TRK Bot | Log Sistemi Kanal ID: ${channel.id}`);
    modlogkanal.send({ embed });
  }
  if (channel.type === "voice") {
    let embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        `<a:hypesqustuck_out_tonguead1:750076071721828452> ${channel.name} **Adlı Ses Kanalı Silindi**`
      )
      .setFooter(`TRK Bot | Log Sistemi  Kanal ID: ${channel.id}`);
    modlogkanal.send({ embed });
  }
      ...
      try {
        if (!message.member.permissions.has("BAN_MEMBERS")) {
          ...
          return message.channel
            .send(
              `<a:rage:750076071721828452> **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`
            )
      ...
      try {
        if (!newMessage.member.permissions.has("BAN_MEMBERS")) {
          ...
          return newMessage.channel
            .send(
              `<a:rage:750076071721828452> **Hey ${newMessage.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`
            )
            .then(message => message.delete(3000));
        }
      } catch (err) {
    ...
    if (
      ...
      try {
        if (!message.member.permissions.has("BAN_MEMBERS")) {
          ...
          return message.channel
            .send(
              `<a:rage:750076071721828452> **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`
            )
            .then(message => message.delete(3000));
        }
      } catch (err) {
    ...
    if (
      ...
      try {
        if (!newMessage.member.permissions.has("BAN_MEMBERS")) {
          ...
          return newMessage.channel
            .send(
              `<a:rage:750076071721828452> **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`
            )
            .then(message => message.delete(3000));
        }
      } catch (err) {
...
client.on("message", async msg => {
  ...
  if (i == "acik") {
    if (
      ...
      try {
        return msg.reply(
          "**<a:blush:750076071721828452> Aleyküm Selam, Hoşgeldin.** "
        );
      } catch (err) {
        console.log(err);
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (member.guild.memberCount == sayaç) {
    return client.channels
      .get(kanal)
      .send(
        `<a:blush:750076071721828452> **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`
      );
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache
      .get(kanal)
      .send(
        "<a:flushed:750076071721828452> **Selam!** `" +
          member.user.username +
